...
sshpass -p "1" ssh -t $REMOTE_HOST "sudo RELEASE_NODE=develop /release/m1x/bin/m1x stop"
sshpass -p "1" ssh -t $REMOTE_HOST "sudo PHX_SERVER=true RELEASE_NODE=develop /release/m1x/bin/m1x daemon"
echo 'begin restart dsa'
sshpass -p "1" ssh -t $REMOTE_HOST "sudo RELEASE_NODE=dsa_1 /release/m1x/bin/m1x stop"
sshpass -p "1" ssh -t $REMOTE_HOST "sudo RELEASE_NODE=dsa_1 DSA_PORT=20081 /release/m1x/bin/m1x daemon"
echo '>>>>  restart finish <<<<'